...
const RESOLVED_RE =
  /(?:^resolved\b|\bpreviously resolved\b|\b(?:is|was|has been|now) (?:fixed|resolved|restored|recovered)\b|\b(?:working again|back online|no action (?:is )?required|for (?:historical )?context only|access (?:has|had|was) (?:been )?(?:removed|revoked)|(?:account|access) (?:has|was) been secured|has been secured|issue (?:is|was) contained|has (?:operated|worked|run|been) (?:normally|correctly|fine|stable) since|(?:renewal|replacement|fix|repair|change|work) (?:has|have|is|was) (?:already )?(?:been )?(?:scheduled|planned|booked))\b)/g;

const REOPENED_RE =
  /\b(?:not (?:fixed|resolved|restored)|still (?:down|failing|failed|broken|blocked|unavailable|has access)|(?:down|failed|failing|broken|blocked|unavailable|stopped|recurred) again|remains? (?:down|broken|blocked|unavailable)|continues? to fail|access (?:has|have) not been revoked)\b/g;
